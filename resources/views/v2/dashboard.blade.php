...
@section('content')
    ...
    <div class="rounded-2xl p-4 sm:p-6 border border-blue-100 mb-6"
        ...
        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div class="flex flex-col gap-1">
                <h1 class="text-[#0d141b] dark:text-white text-2xl font-black tracking-tight">Dashboard Overview
                </h1>
                <p class="text-[#4c739a] text-sm sm:text-base font-normal">
                    Monitor santri evaluation progress and manage SAW criteria.
                    @if($activePeriode)
                        <span
                            class="inline-flex items-center gap-1 ml-2 text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full border border-green-200">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('kriteria.index') }}"
                    class="inline-flex items-center gap-2 rounded-xl bg-white dark:bg-slate-800 px-4 py-2.5 text-sm font-medium text-[#0d141b] dark:text-white shadow-sm ring-1 ring-inset ring-[#e7edf3] dark:ring-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all flex-1 sm:flex-none justify-center">
                    <span class="material-symbols-outlined text-[20px]">settings</span>
                    Configuration
                </a>
                <a href="{{ route('santri.create') }}"
                    class="group inline-flex items-center gap-2 rounded-xl bg-primary hover:bg-primary/90 px-5 py-2.5 text-sm font-bold text-white shadow-lg hover:shadow-xl hover:shadow-primary/30 transform hover:-translate-y-0.5 transition-all duration-200 flex-1 sm:flex-none justify-center">
                    <span
    ...
    <div class="grid grid-cols-1 gap-4 sm:gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Card 1: Total Santri -->
        <div
            class="relative overflow-hidden rounded-xl bg-white dark:bg-slate-900 p-6 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800">
            <dt>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        ...
        <div
            class="lg:col-span-2 rounded-xl bg-white dark:bg-slate-900 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800 p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                <div>
                    <h3 class="text-base font-semibold leading-6 text-[#0d141b] dark:text-white">Top 5 Performing Santri
                    </h3>
                    <p class="mt-1 text-sm text-[#4c739a]">Highest final scores based on SAW calculation.</p>
                </div>
                <a href="{{ route('perhitungan.rekomendasi') }}"
                    class="text-sm font-medium text-primary hover:text-blue-700">View
                    All</a>
            </div>
            ...
            @if($topSantri->count() > 0)
                <div class="relative mt-4 h-64 w-full">
                    <div class="absolute inset-0 flex flex-col justify-between text-xs text-[#4c739a]">
                        <div class="border-b border-slate-100 dark:border-slate-800 w-full h-0 flex items-center">
                            <span>1.0</span>
                        </div>
                        <div class="border-b border-slate-100 dark:border-slate-800 w-full h-0 flex items-center">
                            <span>0.8</span>
                    ...
                    <!-- Bars Container -->
                    <div class="absolute inset-0 flex items-end justify-between pl-8 pb-6 pt-2 gap-2 sm:gap-4">
                        @foreach($topSantri as $index => $riwayat)
                            @php
                                $opacity = 90 - ($index * 10);
                                $heightPercent = min(100, $riwayat->nilai_akhir * 100);
                            @endphp
                            <div class="group relative flex flex-col items-center gap-2 flex-1">
                                <div class="w-full max-w-[64px] bg-primary/{{ $opacity }} hover:bg-primary transition-all duration-300 rounded-t-md relative group-hover:shadow-lg"
                                    style="height: {{ $heightPercent }}%">
                                    <div
                                        class="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900 text-white text-xs px-2 py-1 rounded z-10 whitespace-nowrap">
                                        {{ number_format($riwayat->nilai_akhir, 2) }}
                                    </div>
                                </div>
                                <span
                                    class="text-[10px] sm:text-xs font-medium text-[#4c739a] truncate w-full text-center max-w-[64px]">{{ $riwayat->santri->nama ?? 'N/A' }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <div
            class="lg:col-span-1 rounded-xl bg-white dark:bg-slate-900 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800 p-6 flex flex-col">
            ...
            <div class="flex-1 flex flex-col items-center justify-center">
                ...
                <div class="relative w-48 h-48 rounded-full mb-6"
                    ...
                    <div
                        class="absolute inset-0 m-auto w-32 h-32 bg-white dark:bg-slate-900 rounded-full flex items-center justify-center flex-col shadow-sm">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mt-8">
        ...
        <div class="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <a class="group relative flex flex-col items-start justify-between rounded-xl bg-white dark:bg-slate-900 p-6 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800 hover:ring-primary/50 hover:shadow-md transition-all"
                href="{{ route('santri.index') }}">
                <div>
                    <div
                        class="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 group-hover:text-white group-hover:bg-blue-600 transition-colors">
                ...
                <span
                    class="mt-4 flex items-center text-sm font-medium text-blue-600 dark:text-blue-400 group-hover:text-blue-700">
            ...
            <a class="group relative flex flex-col items-start justify-between rounded-xl bg-white dark:bg-slate-900 p-6 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800 hover:ring-primary/50 hover:shadow-md transition-all"
                ...
                <div>
                    <div
                        class="mb-4 inline-flex h-10 w-10 items-center justify-center rounded-lg bg-purple-50 dark:bg-purple-900/20 text-purple-600 group-hover:text-white group-hover:bg-purple-600 transition-colors">
                ...
                <span
                    class="mt-4 flex items-center text-sm font-medium text-purple-600 dark:text-purple-400 group-hover:text-purple-700">
        ...
        <div
            class="lg:col-span-2 rounded-xl bg-white dark:bg-slate-900 shadow-sm ring-1 ring-[#e7edf3] dark:ring-slate-800 p-0 overflow-hidden">
            <div
                class="border-b border-[#e7edf3] dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 px-6 py-4 flex justify-between items-center">
                <h3 class="text-base font-semibold text-[#0d141b] dark:text-white">Recent Assessments</h3>
                <a href="{{ route('perhitungan.history') }}"
                    class="text-xs font-medium text-primary hover:text-blue-700">View
                    History</a>
            </div>
            <ul class="divide-y divide-[#e7edf3] dark:divide-slate-800" role="list">
                @forelse($recentAssessments as $assessment)
                    <li
                        class="flex items-center gap-x-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer">
                        <div
                            class="h-10 w-10 flex-none rounded-full {{ $assessment->nilai_akhir >= 0.6 ? 'bg-primary/10 text-primary' : 'bg-slate-200 dark:bg-slate-700 text-[#4c739a]' }} flex items-center justify-center text-xs font-bold">
                            {{ strtoupper(substr($assessment->santri->nama ?? 'NA', 0, 2)) }}
                        </div>
                        <div class="min-w-0 flex-auto">
                            <p class="text-sm font-semibold leading-6 text-[#0d141b] dark:text-white">
                                {{ $assessment->santri->nama ?? 'N/A' }}
                            </p>
                            <p class="mt-1 truncate text-xs leading-5 text-[#4c739a]">{{ $assessment->periode->nama ?? 'N/A' }}
                            </p>
                        </div>
                        <div class="flex flex-col items-end">
                            @if($assessment->nilai_akhir >= 0.6)
                                <span
                                    class="inline-flex items-center rounded-md bg-green-50 px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20">Recommended</span>
                            @else
                                <span
                                    class="inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-medium text-red-700 ring-1 ring-inset ring-red-600/10">Not
                                    Rec.</span>
                            @endif
