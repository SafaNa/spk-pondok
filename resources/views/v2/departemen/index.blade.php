                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-4 font-semibold whitespace-nowrap text-center">Jenis Pelanggaran</th>
                        ...
                        @forelse($departemen as $dept)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium {{ $dept->jenis_pelanggaran_count > 0 ? 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' : 'bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400' }}">
                                        {{ $dept->jenis_pelanggaran_count }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div
                                        class="flex items-center justify-end gap-2 opacit-0 group-hover:opacity-100 transition-opacity">
                                        <a href="{{ route('departemen.show', $dept->id) }}"
                                            class="p-2 text-[#4c739a] hover:text-primary transition-colors rounded-lg hover:bg-primary/5"
                                            title="Detail">
                                            <span class="material-symbols-outlined text-[20px]">visibility</span>
                                        </a>
                                        <a href="{{ route('departemen.edit', $dept->id) }}"
