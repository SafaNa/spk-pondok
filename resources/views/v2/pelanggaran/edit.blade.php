...
@section('breadcrumb', 'Pelanggaran / Edit')
...
@section('content')
    <div class="flex flex-col gap-6 w-full mx-auto pb-10">
        ...
        <div
            ...
            {{-- Form --}}
            <form action="{{ route('pelanggaran.update', $violation->id) }}" method="POST" class="p-6 sm:p-10">
                @csrf
                @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="santri_id" required
                                        ...
                                        @foreach ($santri as $s)
                                            <option value="{{ $s->id }}" {{ old('santri_id', $violation->santri_id) == $s->id ? 'selected' : '' }}>
                                                {{ $s->nama }} ({{ $s->nis }})
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="date" name="tanggal_kejadian" value="{{ old('tanggal_kejadian', $violation->tanggal_kejadian->format('Y-m-d')) }}"
                                        required
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="jenis_pelanggaran_id" required
                                        ...
                                        @foreach ($jenisPelanggaran as $jp)
                                            <option value="{{ $jp->id }}"
                                                {{ old('jenis_pelanggaran_id', $violation->jenis_pelanggaran_id) == $jp->id ? 'selected' : '' }}>
                             ...
                             <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <textarea name="sanksi" id="sanksi" rows="3" required
                                        ...
                                        placeholder="Sanksi akan muncul otomatis saat jenis pelanggaran dipilih...">{{ old('sanksi', $violation->sanksi) }}</textarea>
                                </div>
                                <p class="text-xs text-slate-500 mt-1 ml-1">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <textarea name="catatan" rows="4"
                                        class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white placeholder:text-slate-400 font-medium resize-none focus:outline-none focus:border-primary focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                                        placeholder="Tambahkan detail kejadian jika perlu...">{{ old('catatan', $violation->catatan) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Init Choices.js for Jenis Pelanggaran
            ...
            if(jenisSelect) {
                ...
                jenisSelect.addEventListener('change', function(event) {
                    ...
                    // Only update if current value is empty or confirmation/logic? 
                    // Actually user requested: "when selecting..., show default". 
                    // So we overwrite. But in Edit, user might have custom data. 
                    // Let's only overwrite if it REALLY changes. 
