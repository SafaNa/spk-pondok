...
@section('breadcrumb', 'Pelanggaran / Detail')
                ...
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-green-600/80 dark:text-green-400 mt-1 text-sm">
                            Diverifikasi oleh <span class="font-semibold">{{ $violation->verifiedBy->name }}</span>
                            pada {{ $violation->tanggal_verifikasi->format('d F Y, H:i') }}
                        </p>
                    </div>
                </div>
            </div>
        @else
            <div class="bg-gradient-to-r from-red-500/10 to-orange-500/10 rounded-2xl p-6 border border-red-500/20">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
                    ...
                    @if(Auth::user()->isAdmin() || (Auth::user()->isPengurusDepartemen() && $violation->jenisPelanggaran->departemen_id == Auth::user()->departemen_id))
            ...
            <div class="p-6">
                <div class="flex items-center gap-4">
                    <div
                        ...
                        {{ strtoupper(substr($violation->santri->nama, 0, 2)) }}
                    </div>
                    <div>
                        <p class="text-xl font-bold text-[#0d141b] dark:text-white">{{ $violation->santri->nama }}</p>
                        <p class="text-sm text-[#4c739a] mt-1">NIS: {{ $violation->santri->nis }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <p class="font-medium text-[#0d141b] dark:text-white">
                                {{ $violation->tanggal_input->format('d F Y, H:i') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-[#0d141b] dark:text-white">
                        {{ $violation->jenisPelanggaran->nama_pelanggaran }}
                    </p>
                    @if($violation->jenisPelanggaran->deskripsi)
                        <p class="text-sm text-[#4c739a] mt-1">{{ $violation->jenisPelanggaran->deskripsi }}</p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    @php
                        ...
                        $badgeColor = match ($kategori->kode_kategori) {
                            'R' => 'bg-yellow-500/20 text-yellow-700 border-yellow-500/30',
                            'S' => 'bg-orange-500/20 text-orange-700 border-orange-500/30',
                            'B' => 'bg-red-500/20 text-red-700 border-red-500/30',
                    ...
                    <div class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border {{ $badgeColor }}">
                        <span class="font-bold text-lg">{{ $kategori->nama_kategori }}</span>
                        <span class="text-sm">({{ $kategori->bobot_poin }} poin)</span>
                    </div>
                </div>

                <div class="bg-slate-50 dark:bg-slate-800 rounded-lg p-4">
                    <p class="text-sm font-medium text-[#4c739a] mb-2">Sanksi</p>
                    <p class="text-[#0d141b] dark:text-white leading-relaxed">{{ $violation->sanksi }}</p>
                </div>

                @if($violation->catatan)
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                        <p class="text-sm font-medium text-blue-700 dark:text-blue-400 mb-2">Catatan</p>
                        <p class="text-[#0d141b] dark:text-white">{{ $violation->catatan }}</p>
                    </div>
                @endif
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <div
                            ...
                            {{ strtoupper(substr($violation->createdBy->name, 0, 2)) }}
                        </div>
                        <div>
                            <p class="font-medium text-[#0d141b] dark:text-white">{{ $violation->createdBy->name }}</p>
                            <p class="text-xs text-[#4c739a]">{{ $violation->createdBy->email }}</p>
                        </div>
                    </div>
                </div>
            </div>
