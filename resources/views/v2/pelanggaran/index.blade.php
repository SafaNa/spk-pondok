    ...
    <div class="flex flex-col gap-6" x-data="{ showVerifyModal: false, verifyActionUrl: '' }">
        ...
        <div class="rounded-2xl p-4 sm:p-6 border border-blue-100 mb-6"
            ...
            <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                ...
                @if(!Auth::user()->isPengurusPerizinan())
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-2">
                                    <div
                                        class="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-sm">
                                        {{ strtoupper(substr($violation->santri->nama, 0, 2)) }}
                                    </div>
                                    <div>
                                        <p class="font-bold text-[#0d141b] dark:text-white">{{ $violation->santri->nama }}</p>
                                        <p class="text-xs text-[#4c739a]">{{ $violation->santri->nis }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="ml-13 space-y-2">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span
                                            class="text-sm font-medium text-[#0d141b] dark:text-white">{{ $violation->jenisPelanggaran->nama_pelanggaran }}</span>
                                        @php
                                            $kategori = $violation->jenisPelanggaran->kategoriPelanggaran;
                                            $badgeColor = match ($kategori->kode_kategori) {
                                                'R' => 'bg-yellow-500/10 text-yellow-700',
                                                'S' => 'bg-orange-500/10 text-orange-700',
                                    ...
                                    <p class="text-sm text-[#4c739a]">
                                        <span class="font-medium">Sanksi:</span> {{ Str::limit($violation->sanksi, 80) }}
                                    </p>
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                @if(Auth::user()->isAdmin() || (Auth::user()->isPengurusDepartemen() && $violation->jenisPelanggaran->departemen_id == Auth::user()->departemen_id))
                                    @if(Route::has('pelanggaran.edit') && $violation->status_sanksi !== 'selesai')
                                        <div class="flex gap-2">
                                            ...
                                            <form action="{{ route('pelanggaran.destroy', $violation->id) }}" method="POST"
                                    ...
                                    @if($violation->status_sanksi === 'belum_selesai')
                                        <button type="button"
                                            @click="verifyActionUrl = '{{ route('pelanggaran.verify', $violation->id) }}'; showVerifyModal = true"
