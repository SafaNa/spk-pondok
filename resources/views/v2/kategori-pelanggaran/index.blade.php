...
@section('breadcrumb', 'Pelanggaran / Kategori')
...
@section('content')
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-4 font-semibold whitespace-nowrap w-24">Kode</th>
                    ...
                    <tbody class="divide-y divide-[#e7edf3] dark:divide-slate-700">
                        @forelse($kategori as $cat)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-semibold text-[#0d141b] dark:text-white">{{ $cat->nama_kategori }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        ...
                                        <button
                                            onclick="openEditModal('{{ $cat->id }}', '{{ $cat->kode_kategori }}', '{{ $cat->nama_kategori }}', '{{ $cat->bobot_poin }}')"
                                        ...
                                        <form action="{{ route('kategori-pelanggaran.destroy', $cat->id) }}" method="POST"
                                            class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                @click.prevent="$store.deleteModal.open($el.closest('form'), 'Yakin ingin menghapus kategori {{ $cat->nama_kategori }}?')"
                                                type="button"
    ...
    <x-modal id="createModal" title="Tambah Kategori Pelanggaran">
        <form action="{{ route('kategori-pelanggaran.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Kode Kategori</label>
                <input type="text" name="kode_kategori" required maxlength="10"
            ...
            <div>
                ...
                <input type="number" name="bobot_poin" required min="0" value="0"
                    class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:ring-primary focus:border-primary">
            </div>
    ...
    <x-modal id="editModal" title="Edit Kategori Pelanggaran">
        <form id="editForm" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Kode Kategori</label>
                <input type="text" name="kode_kategori" id="edit_kode" required maxlength="10"
                    class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:ring-primary focus:border-primary">
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nama Kategori</label>
            ...
            <div>
                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Bobot Poin</label>
                <input type="number" name="bobot_poin" id="edit_bobot_poin" required min="0" value="0"
                    class="w-full rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:ring-primary focus:border-primary">
            </div>
            <div class="flex justify-end gap-3 mt-6">
    ...
    <script>
        function openEditModal(id, kode, nama, bobot) {
            document.getElementById('editForm').action = '/kategori-pelanggaran/' + id;
