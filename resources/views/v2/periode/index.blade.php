...
@section('content')
        ...
        @if($activePeriode)
            <div
                ...
                <div class="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between relative z-10">
                    ...
                    <button
                        onclick="openEditModal('{{ $activePeriode->id }}', '{{ $activePeriode->nama }}', '{{ $activePeriode->keterangan }}')"
        ...
        <div
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($periodes as $periode)
                            <tr class="group hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="text-sm text-[#4c739a]">{{ $periode->keterangan ? Str::limit($periode->keterangan, 50) : '-' }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        ...
                                        <button
                                            onclick="openEditModal('{{ $periode->id }}', '{{ $periode->nama }}', '{{ $periode->keterangan }}')"
            ...
            @if($periodes->hasPages())
                <div class="px-6 py-4 border-t border-[#e7edf3] dark:border-slate-700 flex items-center justify-between">
                    <span class="text-sm text-[#4c739a]">Menampilkan {{ $periodes->firstItem() }}-{{ $periodes->lastItem() }}
                        dari {{ $periodes->total() }} periode</span>
    ...
    <script>
        ...
        function openEditModal(id, nama, keterangan) {
            document.getElementById('editForm').action = '/periode/' + id;
