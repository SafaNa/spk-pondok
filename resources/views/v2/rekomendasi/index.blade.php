    ...
    <div class="rounded-2xl p-4 sm:p-6 border border-blue-100 mb-6"
        ...
        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
            <div class="flex flex-col gap-1">
                <h1 class="text-[#0d141b] dark:text-white text-2xl font-black tracking-tight">Hasil Analisis SAW</h1>
                <p class="text-[#4c739a] text-sm sm:text-base font-normal">
                    ...
                    @if($periode)
                        <span
                            class="inline-flex items-center gap-1 ml-2 text-xs bg-white/50 dark:bg-slate-800/50 border border-blue-500/20 text-blue-600 px-2 py-0.5 rounded-full font-medium">
                            {{ $periode->nama }}
                        </span>
                    @endif
                </p>
                ...
                <thead>
                    <tr class="bg-[#f8fafc] dark:bg-slate-800/50 border-b border-[#e7edf3] dark:border-slate-700">
                        ...
                        <th class="p-4 text-xs font-semibold tracking-wide text-[#4c739a] uppercase w-32 whitespace-nowrap">
                            Nilai Akhir</th>
                        <th class="p-4 text-xs font-semibold tracking-wide text-[#4c739a] uppercase w-40 whitespace-nowrap">
                            Status</th>
                        <th
                            class="p-4 text-xs font-semibold tracking-wide text-[#4c739a] uppercase w-24 text-center whitespace-nowrap">
                            Actions</th>
                    </tr>
                </thead>
                    ...
                    @forelse($riwayat as $index => $r)
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                            <td class="p-4">
                                @if($rank <= 3)
                                    <span
                                        class="flex items-center justify-center w-8 h-8 rounded-full {{ $rank == 1 ? 'bg-amber-100 text-amber-600' : ($rank == 2 ? 'bg-slate-200 text-slate-600' : 'bg-orange-100 text-orange-600') }} font-bold text-sm">
                                        {{ $rank }}
                                    </span>
                                @else
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="h-10 w-10 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">
                                        {{ strtoupper(substr($r->santri->nama ?? 'NA', 0, 2)) }}
                                    </div>
                                    <div>
                                        <p class="font-semibold text-[#0d141b] dark:text-white">{{ $r->santri->nama ?? 'N/A' }}
                                        </p>
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        class="font-bold text-[#0d141b] dark:text-white">{{ number_format($nilai, 2, ',', '.') }}</span>
                                </div>
                            </td>
                            <td class="p-4">
                                <span
                        ...
                        <tr>
                            <td colspan="5" class="p-8 text-center text-[#4c739a]">
                                ...
                                <a href="{{ route('perhitungan.index') }}"
                                    class="text-primary hover:underline text-sm mt-2 inline-block">Mulai perhitungan</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
