    ...
    <div class="space-y-8">
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <div class="border-t border-gray-100 pt-6">
                <form action="{{ route('perhitungan.history') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                    <!-- Filter Periode -->
        ...
        <div class="space-y-10">
            ...
            @else
                @foreach($riwayat as $periodeId => $items)
                    @php
                        ...
                        <div class="flex items-center justify-between mb-4 border-l-4 border-[var(--color-primary-600)] pl-4">
                            <div>
                                <h4 class="text-xl font-bold text-gray-900">
                                    {{ $periode ? $periode->nama : 'Periode Tidak Diketahui' }}
                                </h4>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50/50">
                                        <tr>
                                            ...
                                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Santri</th>
                                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nilai Akhir</th>
                                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tanggal Hitung</th>
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors group">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <div class="flex items-center">
                                                        <div class="h-8 w-8 rounded-full bg-[var(--color-primary-100)] flex items-center justify-center text-[var(--color-primary-600)] font-bold text-[10px] mr-3">
                                                            {{ collect(explode(' ', $item->santri->nama))->map(fn($word) => substr($word, 0, 1))->take(2)->implode('') }}
                                                        </div>
                                                        <div>
                                                            <div class="text-sm font-semibold text-gray-900 group-hover:text-[var(--color-primary-600)] transition-colors">
                                                                {{ $item->santri->nama }}
                                                            </div>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold {{ $item->nilai_akhir > 0.8 ? 'bg-green-100 text-green-700' : ($item->nilai_akhir > 0.5 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
