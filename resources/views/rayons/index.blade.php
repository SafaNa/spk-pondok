...
@section('title', 'Data Rayon - Santri Admin')
@section('mobile_title', 'Rayon')
@section('breadcrumb', 'Data Rayon')
...
@section('content')
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-4 whitespace-nowrap">Nama Rayon</th>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($rayons as $rayon)
                            <tr class="group hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm font-bold text-[#0d141b] dark:text-white">{{ $rayon->name }}</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button onclick="openEditModal('{{ $rayon->id }}', '{{ $rayon->name }}')"
                                        ...
                                        <div x-data>
                                            <form action="{{ route('rayons.destroy', $rayon->id) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button
                                                    @click.prevent="$store.deleteModal.open($el.closest('form'), 'Yakin ingin menghapus rayon {{ $rayon->name }}?')"
                                                    type="button"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-8 text-center text-[#4c739a]">
                                    <span class="material-symbols-outlined text-3xl mb-2">apartment</span>
                                    <p>Belum ada data rayon</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="addModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
            <div class="px-6 py-4 border-b border-[#e7edf3] dark:border-slate-700 flex items-center justify-between">
                <h3 class="text-lg font-bold text-[#0d141b] dark:text-white">Tambah Rayon Baru</h3>
            ...
            <form action="{{ route('rayons.store') }}" method="POST">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-[#0d141b] dark:text-white mb-1">Nama Rayon <span
    ...
    <div id="editModal" class="fixed inset-0 bg-black/50 z-50 hidden items-center justify-center">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
            <div class="px-6 py-4 border-b border-[#e7edf3] dark:border-slate-700 flex items-center justify-between">
                <h3 class="text-lg font-bold text-[#0d141b] dark:text-white">Edit Rayon</h3>
