...
@section('title', 'Edit Kategori Kepulangan')
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800">
            <div class="bg-gradient-to-br from-primary/10 via-purple-500/5 to-pink-500/5 px-6 py-6 sm:px-8 sm:py-8 border-b border-primary/10">
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold mb-2 text-slate-900 dark:text-white tracking-tight">Edit Kategori Kepulangan</h1>
                        <p class="text-slate-500 dark:text-slate-400 text-base max-w-xl">Perbarui data kategori <span class="font-semibold text-primary">{{ $leaveCategory->name }}</span>.</p>
                    </div>
                </div>
            </div>

            <form action="{{ route('admin.leave-categories.update', $leaveCategory) }}" method="POST"
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <input type="text" name="name" value="{{ old('name', $leaveCategory->name) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group">
                                ...
                                <input type="number" name="order" value="{{ old('order', $leaveCategory->order) }}" min="0"
                                    class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white placeholder:text-slate-400 font-medium focus:outline-none focus:border-primary focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-primary/10 transition-all duration-200">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <input type="text" name="max_duration" value="{{ old('max_duration', $leaveCategory->max_duration) }}"
                                class="w-full pl-12 pr-4 py-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white placeholder:text-slate-400 font-medium focus:outline-none focus:border-primary focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-primary/10 transition-all duration-200"
                                placeholder="Contoh: Maksimal 2 hari 2 malam">
                        </div>
                    </div>

                    <div x-data="{ fixed: {{ old('is_fixed_duration', $leaveCategory->is_fixed_duration) ? 'true' : 'false' }} }" class="space-y-3">
                        <div class="flex items-start gap-3 p-4 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800">
                            <input type="checkbox" name="is_fixed_duration" id="is_fixed_duration" value="1"
                                {{ old('is_fixed_duration', $leaveCategory->is_fixed_duration) ? 'checked' : '' }}
                                x-model="fixed"
                        ...
                        <div x-show="fixed" x-collapse>
                            <div class="space-y-2">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="number" name="duration_days" value="{{ old('duration_days', $leaveCategory->duration_days) }}" min="1"
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative group">
                            ...
                            <textarea name="notes" rows="2"
                                ...
                                placeholder="Catatan tambahan untuk kategori ini">{{ old('notes', $leaveCategory->notes) }}</textarea>
                        </div>
                    </div>
