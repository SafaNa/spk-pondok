...
@section('content')
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-[#e7edf3] dark:border-slate-700 overflow-hidden">
        <form action="{{ isset($kriterium) ? route('kriteria.update', $kriterium->id) : route('kriteria.store') }}" method="POST" class="p-6 md:p-8 space-y-8">
            @csrf
            @isset($kriterium)
                @method('PUT')
            @endisset
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:col-span-3 space-y-2">
                    ...
                    <div class="relative">
                        <input name="kode_kriteria" value="{{ old('kode_kriteria', $kriterium->kode_kriteria ?? '') }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="relative">
                        ...
                        <div class="absolute inset-y-0 right-0 flex items-center px-4 bg-[#f6f7f8] dark:bg-slate-700 border-l border-[#e7edf3] dark:border-slate-600 rounded-r-lg text-[#4c739a] font-medium">
                            %
                        </div>
                    </div>
                    <!-- Helper / Validation Visualization -->
                    <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 border border-blue-100 dark:border-blue-800/50 flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 mb-1">
                                <div class="{{ $totalBobot >= 100 ? 'bg-red-500' : ($totalBobot >= 75 ? 'bg-amber-500' : 'bg-emerald-500') }} h-2 rounded-full" style="width: {{ $totalBobot }}%"></div>
                            </div>
                            <p class="text-xs text-[#4c739a]">
                                Current total used: <span class="font-bold text-[#0d141b] dark:text-white">{{ $totalBobot }}%</span>.
                                You have <span class="font-bold {{ $remainingBobot > 0 ? 'text-primary' : 'text-red-500' }}">{{ $remainingBobot }}%</span> remaining.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <label class="cursor-pointer group relative">
                            <input {{ old('jenis', $kriterium->jenis ?? 'benefit') == 'benefit' ? 'checked' : '' }} class="peer sr-only" name="jenis" type="radio" value="benefit" />
                            <div class="p-4 rounded-xl border-2 border-[#e7edf3] dark:border-slate-600 bg-white dark:bg-slate-800 hover:border-primary/50 transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:shadow-sm">
                        ...
                        <!-- Cost Option -->
                        <label class="cursor-pointer group relative">
                            <input {{ old('jenis', $kriterium->jenis ?? '') == 'cost' ? 'checked' : '' }} class="peer sr-only" name="jenis" type="radio" value="cost" />
                            <div class="p-4 rounded-xl border-2 border-[#e7edf3] dark:border-slate-600 bg-white dark:bg-slate-800 hover:border-primary/50 transition-all peer-checked:border-primary peer-checked:bg-primary/5 peer-checked:shadow-sm">
                                <div class="flex items-center gap-3 mb-2">
            ...
            <!-- Action Bar -->
            <div class="bg-[#f6f7f8] dark:bg-slate-800 -mx-8 -mb-8 px-6 py-4 md:px-8 border-t border-[#e7edf3] dark:border-slate-700 flex flex-col-reverse md:flex-row items-center justify-end gap-3 mt-8">
                <a href="{{ route('kriteria-v2') }}"
